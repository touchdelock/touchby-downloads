...
const home=(u,faq)=>`<div class="tb-root"><section class="tb-home-hero"><h1><span>スマホを置く、</span><span>朝をはじめる。</span></h1><div class="tb-home-intro"><p>スマホを置く、<br>ベッドから起きる、<br>そのきっかけを、一枚のカードに。</p><a class="tb-text-link" href="#tb-products">2つの商品を見る<span aria-hidden="true">↓</span></a></div></section><section class="tb-section tb-product-section" id="tb-products"><div class="tb-section-label"><h2>ふたつの商品</h2></div><div class="tb-products">${product('lock',u)}${product('alarm',u)}</div></section><section class="tb-how"><h2>3ステップで使えます</h2><div class="tb-steps"><div class="tb-step"><span>01</span><div><h3>カードを選ぶ</h3><p>アプリの利用を区切りたいときはロック。朝、起きるきっかけにはアラーム。商品に対応する専用アプリを用意します。</p></div></div><div class="tb-step"><span>02</span><div><h3>置く場所を決める</h3><p>机、リビング、玄関。ロック用は手を伸ばさないと届かない場所に。アラーム用はベッドから離れた場所に。</p></div></div><div class="tb-step"><span>03</span><div><h3>アプリで読み取る</h3><p>初回の設定を済ませたら、カードにタッチ。読み取り方や必要な許可は、端末と商品によって異なります。</p></div></div></div></section><section class="tb-faq"><div class="tb-faq-intro"><h2>よくあるご質問</h2><a class="tb-text-link" href="${u('/inquiry')}">お問い合わせ<span aria-hidden="true">↗</span></a></div><div>${faq}</div></section>${cta(u)}</div>`;
...
function boot(){
 ...
 try{
  if(kind==='home'){
   ...
   const detailTitles={
    '選ぶ。かざす。集中する。':'選ぶ、かざす、集中する。',
    'ご購入前に、知っておきたいこと。':'購入前に知っておきたいこと'
   };
   main.querySelectorAll('[data-parts=title] [class*=title_title]').forEach(heading=>{
    const replacement=detailTitles[heading.textContent.trim()];
    if(replacement)heading.textContent=replacement;
   });
   main.querySelectorAll('[class*=column-image-and-text_textBody]').forEach(body=>{
    // BASE rich text uses separate paragraphs for labels, copy and blank lines.
    // Keep the copy and inline markup; let CSS provide consistent spacing.
    body.querySelectorAll(':scope > p').forEach(p=>{
     const text=p.textContent.trim();
     if(!text&&!p.querySelector('img,video,iframe,input')){p.remove();return;}
     const strong=p.querySelector(':scope > strong');
     if(strong&&strong.textContent.trim()===text)p.classList.add('tb-copy-heading');
    });
   });
  ...
  document.documentElement.dataset.tbDesign='2026-09-25.7';
