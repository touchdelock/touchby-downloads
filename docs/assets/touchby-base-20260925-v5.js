...
const product=(k,u)=>`<article class="tb-product tb-product--${k}"><a class="tb-product-visual" href="${u(k==='lock'?'/p/00005':'/p/00004')}" aria-label="${name(k)}の詳細">${card(k)}</a><div class="tb-product-heading"><h3>${name(k)}</h3></div><p class="tb-product-desc">${k==='lock'?'使わないアプリを選ぶ。カードにタッチしてロック。<br>もう一度タッチすると、解除できます。':'ベッドから離れた場所にカードを置く。<br>読み取りに行くことが、起きるきっかけに。'}</p><a class="tb-text-link" href="${u(k==='lock'?'/p/00005':'/p/00004')}">詳しく見る<span aria-hidden="true">↗</span></a></article>`;
...
function boot(){
 ...
 try{
  ...
  document.documentElement.dataset.tbDesign='2026-09-25.5';
